...
public class ClientFilter implements ClientRequestFilter, ClientResponseFilter  {
    ...
    @Override
    public void filter(ClientRequestContext requestContext, ClientResponseContext responsContext) throws IOException {
        try {
            Span span = (Span) requestContext.getProperty(CLIENT_SPAN_CONTEXT_KEY);
            if (span != null) {
                Tags.HTTP_STATUS.set(span, responsContext.getStatus());
                span.finish();
            }
        }
        catch (Exception e) {
